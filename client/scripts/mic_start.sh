#!/bin/bash
# Скрипт для начала записи микрофона через системные горячие клавиши
...
log_message "Запуск mic_start.sh"
...
log_message "Выполнение: $PYTHON_EXE main.py --start"
result=$("$PYTHON_EXE" main.py --start 2>&1)
exit_code=$?
