#!/bin/bash
# Скрипт для переключения записи микрофона через системные горячие клавиши
...
log_message() {
    echo "$(date '+%Y-%m-%d %H:%M:%S') [TOGGLE] $1" >> "$LOG_FILE"
}
...
log_message "Запуск mic_toggle.sh"
...
log_message "Выполнение: $PYTHON_EXE main.py --toggle"
result=$("$PYTHON_EXE" main.py --toggle 2>&1)
exit_code=$?
